/* Internal Medicine 2026 — ABIM-Style Exam Simulator */

const ACCESS_CODE  = "IM9000";
const EXAM_SECONDS = 6 * 60 * 60;   // 6 hours for 300 questions
const PASSING_PCT  = 70;
const STORAGE_KEY  = "im2026_exam_state_v1";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Always show the gate on every page load — state is restored only after correct code
...
function shuffleExamQuestions() {
  // QUESTIONS is defined in questions.js as const QUESTIONS = [...]
  // Each question: { id, chapter, domain, question, options:{A,B,C,D}, answer, explanation }
  ...
  questions = shuffledOrder.map((q, idx) => {
    // Randomize which letter maps to each option text
...
function bindGate() {
  ...
  function attempt() {
    ...
    if (val === ACCESS_CODE) {
      errorEl.textContent = "";
      restoreState();   // load any saved session now that the code is verified
      ...
      if (state.phase === "results") {
        ...
        // No saved session (or stale) — start a fresh exam
        shuffleExamQuestions();
        state = {
          phase:     "exam",
          answers:   {},
...
function showBlockModal(unansweredNums) {
  ...
  document.getElementById("sm-title").textContent = `Cannot Submit — ${unansweredNums.length} Question${unansweredNums.length > 1 ? "s" : ""} Unanswered`;
...
function showResults(timedOut = false) {
  ...
  // Sort domains by total questions descending
  const domainOrder = Object.keys(domainStats).sort(
    (a, b) => domainStats[b].total - domainStats[a].total
  );

  const palette = [
    "#1B3A6B","#0A7E8C","#C05A10","#5B1E8A","#1A7340","#8A3030",
    "#0A5C8C","#8A6C10","#2D5A8A","#6B1B3A","#1B6B5A","#5A5B1E",
    "#8A2D1B","#1B4A8A",
  ];
